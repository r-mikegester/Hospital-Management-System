...
// Handle form submissions for adding, editing, or deleting inventory items
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'add') {
        try {
            ...
            die("Error adding asset: " . $e->getMessage());
        }
    }
...
// Fetch all assets with warehouse info
try {
...
<body>
    <div class="relative min-h-screen max-h-screen overflow-auto flex flex-col">
        ...
        <div class="flex flex-1">
            ...
            <main class="flex-1 mt-20 p-6 overflow-auto h-full">
                <h1 class="text-3xl font-bold mb-10">Inventory</h1>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-warning btn-sm"
                                        onclick="editAsset(<?= $asset['asset_id'] ?>, '<?= htmlspecialchars($asset['asset_name'], ENT_QUOTES) ?>', '<?= htmlspecialchars($asset['category'], ENT_QUOTES) ?>', '<?= htmlspecialchars($asset['status'], ENT_QUOTES) ?>', '<?= $asset['acquisition_date'] ?>', '<?= htmlspecialchars($asset['location'], ENT_QUOTES) ?>', <?= $asset['warehouse_id'] ?>, '<?= htmlspecialchars($asset['project_status'], ENT_QUOTES) ?>')">
                                        Edit
                                    </button>
                                    <a href="?delete_id=<?= $asset['asset_id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this item?')">
                                        Delete
                                    </a>
                                </td>
                            </tr>
                ...
                <form method="POST" class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="warehouse_id" id="edit-warehouse-id" class="form-select" required>
                                <?php foreach ($warehouses as $warehouse): ?>
                                    <option value="<?= $warehouse['warehouse_id'] ?>"><?= htmlspecialchars($warehouse['location']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="project_status" id="edit-project-status" class="form-select" required>
                                <option value="Pending">Pending</option>
                                <option value="Completed">Completed</option>
                            </select>
                        </div>
